module TasksHelper
  ...
  def previous_task(index)
    if index >= 0
      link_to "", project_task_path(task.project, tasks[index]), class: "bi bi-chevron-up arrow"
    else
  ...
  def next_task(index)
    if index < tasks.size
      link_to "", project_task_path(task.project, tasks[index]), class: "bi bi-chevron-down arrow"
    else
